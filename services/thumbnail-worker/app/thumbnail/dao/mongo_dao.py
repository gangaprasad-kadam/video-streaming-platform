...
def _get_collection():
    """Return the MongoDB collection used for thumbnail processing logs.
...
async def log_processing_event(video_id: str, event: str, details: dict) -> None:
    """Insert a thumbnail processing log entry into MongoDB.

    Errors from MongoDB are caught and logged as warnings so that logging
    failures never interrupt the main thumbnail pipeline.

    Args:
        video_id: ID of the video this log entry belongs to.
        event: Short event name, e.g. ``"thumbnail_started"`` or
            ``"thumbnail_failed"``.
